...
class Products {
  async getProducts() {
    try {
      let result = await fetch("myProducts.json");
...
class UI {
  displayProducts(data) {
    ...
    data.forEach((item) => {
      ...
      <div class="pocket">
            <img src="${item.imgSrc}" alt="" class="items">
            <h5 class="goods">${item.name}</h5>
            <p class="para3"><span>&#8358</span>${item.price}</p>
    ...
    div.innerHTML = ` <img class ="thumbnail-in-cart" src="${item.imgSrc}"/>
                        <span class="product-in-cart-name">${item.name}</span>
                            <div class="increase_decrease-amount">
                                <button class="increase-amount" style="color: white; font-weight: bold;" data-id=${item.id}>+</button>
                                <p class="item-amount">${item.amount}</p>
                                <button class="decrease-amount" style="color: white; font-weight: bold;" data-id=${item.id}>-</button>
                         </div>
